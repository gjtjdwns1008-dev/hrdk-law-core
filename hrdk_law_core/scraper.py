...
def fetch_law_by_name(api_key: str, law_name: str) -> dict | None:
    """
    [일회성 보강용] 법령'명'으로 법제처에서 현행 법령 본문을 조회합니다.
    get_base_laws는 시행일(efYd)로 조회하지만, 이 함수는 법령명(query)으로 조회합니다.

    Returns: {법령명, 시행일자, 소관부처, 원본, 링크} 또는 None(못 찾음/연결 실패)
    """
    session = _build_session()
    # 1) 목록 검색 (법령명으로)
    try:
        search_url = (
            f"https://www.law.go.kr/DRF/lawSearch.do"
            f"?OC={api_key}&target=law&type=XML&query={law_name}&display=20&page=1"
        )
        ...
        # 검색 결과 중 법령명이 가장 비슷한 것 선택 (정확 일치 우선)
        def _norm(s): return re.sub(r"[\s,·\.]", "", s or "")
        target_norm = _norm(law_name)
        best = None
        for node in law_nodes:
            nm = node.findtext("법령명한글", "").strip()
            mst = node.findtext("법령일련번호", "") or node.findtext("MST", "")
            if not mst:
                continue
            if _norm(nm) == target_norm:
                best = (nm, mst, node); break
            if best is None and target_norm in _norm(nm):
                best = (nm, mst, node)
        if best is None:
            best = (law_nodes[0].findtext("법령명한글", "").strip(),
                    law_nodes[0].findtext("법령일련번호", "") or law_nodes[0].findtext("MST", ""),
                    law_nodes[0])
        found_name, mst_id, node = best
        if not mst_id:
            return None
        enforce_date = node.findtext("시행일자", "")
        ministry = node.findtext("소관부처명", "") or node.findtext("소관부처", "")
        base_link = f"https://www.law.go.kr/법령/{found_name}"
    except Exception as e:
        print(f"  ⚠️ '{law_name}' 목록 검색 실패: {e}")
        return None
    ...
    try:
        detail_url = (
            f"https://www.law.go.kr/DRF/lawService.do"
            f"?OC={api_key}&target=law&type=XML&MST={mst_id}"
        )
        dresp = session.get(detail_url, headers=HEADERS, timeout=30)
        if dresp.status_code != 200 or not dresp.text.strip():
            return None
        droot = ET.fromstring(dresp.text)

        body_parts = []
        for jomun in droot.findall(".//조문단위"):
            if jomun.attrib.get("조문여부") == "조문":
                title = (jomun.findtext("조문제목", "") or "")
                content = (jomun.findtext("조문내용", "") or "")
                if content:
                    body_parts.append(clean_to_markdown(title, content))
        full_text = "\n".join(body_parts)
        if not full_text:
            # 조문단위가 없으면 조문내용 전체를 긁음
            full_text = "\n".join(
                j.text.strip() for j in droot.findall(".//조문내용") if j.text
            )
        stars = "\n".join(s.text.strip() for s in droot.findall(".//별표내용") if s.text)
        if stars:
            full_text += f"\n\n### ⭐ 별표(자격 기준 등)\n{stars}"
